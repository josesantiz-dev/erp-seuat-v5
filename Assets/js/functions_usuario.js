...
//Funcion cuando carga el documento
    ...
    fetch(url)
        ...
        .then((response) => {
            if(response.imagen == 'NULL'){
                var urlimagen = base_url+"/Assets/images/img/logo-empty.png";
...
formEditUsuario.onsubmit = function (e) 
{
    ...
    if (
        ...
        Swal.fire({
            icon: "error",
            title: "Campo vacío",
            text: "Totos los campos son obligatorios!",
        });
        return false;
    }
    let request = window.XMLHttpRequest
